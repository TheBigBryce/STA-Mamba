...
class StokenAttentionLayer(nn.Module):
    def __init__(self, dim, n_iter, stoken_size,
                 ...
                 dt_rank=32, dim_inner=512, d_state=256):
        super().__init__()
        ...
        # Replace StokenAttention with VisionEncoderMambaBlock
        self.attn = VisionEncoderMambaBlock(
            dim=dim,
            dt_rank=dt_rank,
            dim_inner=dim_inner,
            d_state=d_state
    ...
    def forward(self, x):
        ...
        # Reshape to (B, S, C) where S = H * W, suitable for VisionEncoderMambaBlock
        x = x.flatten(2).transpose(1, 2)  # Shape: (B, H*W, C)

        if self.layerscale:
            # Applying Mamba Block with layer scaling
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x)))
            x = x + self.drop_path(self.gamma_2 * self.mlp2(self.norm2(x.transpose(1, 2).reshape(B, C, H, W))))
        else:
            # Applying Mamba Block without layer scaling
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x = x + self.drop_path(self.mlp2(self.norm2(x.transpose(1, 2).reshape(B, C, H, W))))

        # Reshape back to (B, C, H, W) for the next layers
...
class STViT(nn.Module):
    def __init__(self, in_chans=1, num_classes=1000,
        ...
        # self.patch_embed_Unet = PatchEmbed(in_chans, embed_dim[0])
    ...
    def forward(self, x):
        x, skip_out = self.forward_features(x)
        # x = self.head(x)
        return x, skip_out
